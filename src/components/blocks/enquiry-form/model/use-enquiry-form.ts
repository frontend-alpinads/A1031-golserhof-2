...
export function useEnquiryForm({
  ...
  useEffect(() => {
    if (prefilledRoom) {
      // Create a room selection object that matches the expected format
      const roomSelection = {
        selectedRooms: [
          {
            id: prefilledRoom.id,
            room: prefilledRoom,
            guests: guestSelection.adults,
            children: guestSelection.children,
            childAges: guestSelection.childAges.map((age) => ({ age })),
            boardOption: "half-board" as const, // Changed from half-board - not available per website
          },
        ],
      };
